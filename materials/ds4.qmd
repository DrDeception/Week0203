...
subtitle: "Data Visualisation Cheat Sheet"
...
1.  **What type of chart would you use to display the frequency of different job titles (e.g., teacher, engineer, doctor)?**
    -   ANSWER HERE
2.  **How would you visualize the relationship between a person’s age and their income?**
    -   ANSWER HERE
3.  **If you were comparing the median salary of employees across different education levels (e.g., high school, bachelor’s, master’s), which chart type would you use?**
    -   ANSWER HERE
4.  **Which chart would best show the distribution of exam scores in a class?**
    -   ANSWER HERE
5.  **What chart would you use to see how sales numbers have changed over the last 10 years?**
    -   ANSWER HERE
...
# Create a single radar plot for TIPI personality traits for everyone
# Prepare data for radar plot
all_data <- data %>% select(E_TIPI, A_TIPI, C_TIPI, ES_TIPI, O_TIPI) %>% summarise_all(mean, na.rm = TRUE)

# Check if there is enough data for radar plot and no NaN values
if (nrow(all_data) > 0 && all(!is.na(all_data))) {
  # Combine data and set up for radar plot
  radar_data_all <- rbind(rep(7, 5), rep(1, 5), all_data)
  colnames(radar_data_all) <- c("Extraversion", "Agreeableness", "Conscientiousness", "Emotional Stability", "Openness")

  # Create radar plot for everyone
  radarchart(radar_data_all,
             axistype = 1,
             pcol = "blue",
             pfcol = rgb(0.2, 0.5, 0.8, 0.5),
             plwd = 2,
             title = "Personality Profile for All Participants",
             cglcol = "grey", cglty = 1, axislabcol = "grey", caxislabels = seq(1, 7, 1), cglwd = 0.8)
} else {
  cat("[DATA NOT ENOUGH or NaN values present] for overall radar plot\n")
}
